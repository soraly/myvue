...
export function init(
  ...
  function patchVnode(
    ...
    //新node的text为undefined，vnode.children 一般有值
    if (isUndef(vnode.text)) {
      //新旧都有children，进行children的比较
      if (isDef(oldCh) && isDef(ch)) {
        ...
        //新旧都没有children，旧的有text，清空text
      } else if (isDef(oldVnode.text)) {
        api.setTextContent(elm, "");
      }

      //新node的text有值，（新node.children 无值），而且oldVnode的text和新node的text不相等，则删除旧的node，设置新的text
  ...
  return function patch(
    ...
    //判断传入的参数是vnode还是dom，如果传入dom，则生成一个空的vnode，
    //再关联到这个 DOM 元素
    ...
    //判断两个vnode是否相同，需要key 和 sel 都相等，
    //才调用patchVnode进行下一步的比较。否则删除旧的vnode重建
    //当两个vnode的key都没传，则undefined === undefined ，还是为true
